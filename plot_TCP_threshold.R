...
for (m in model_vt){
  ...
  for (K in Nclass_vt){
    load(paste0(m, "_measure258_state_Nclass", K, ".RData"))
    ...
    bitmap(paste0("plots_measure258/plots_state/", m, "_map_Nclass", K, "_state.jpeg"),
           width = 4, height = 3.2, units = "in", type = "jpeg", res = 1000)
    sp_plot <- spplot(shp_plot, zcol = "grp_val",
                      main = paste0("State Map, ATCP = ", format(round(mean(measure_state_dt$TCP), 2), nsmall = 2)),
                      xlab = "", ylab = "",
    ...
    bitmap(paste0("plots_measure258/plots_state/", m, "_stack_hist_Nclass", K, "_state.jpeg"),
