...
quant_val_vt <- (quant_vt[1:Nclass]+quant_vt[2:(Nclass+1)])/2

L_vt_state <- quantile(as.numeric(state_postsamp_mt), quant_vt)
L_vt_state[1] <- 0
L_vt_state[Nclass+1] <- 1
L_val_vt_state <- quantile(as.numeric(state_postsamp_mt), quant_val_vt)
L_dt_state <- data.table(grp = paste0("X", 1:Nclass),
                         grp_low = L_vt_state[1:Nclass],
                         grp_up = L_vt_state[2:(Nclass+1)],
                         grp_val = L_val_vt_state)

L_vt_lga <- quantile(as.numeric(lga_postsamp_mt), quant_vt)
L_vt_lga[1] <- 0
L_vt_lga[Nclass+1] <- 1
L_val_vt_lga <- quantile(as.numeric(lga_postsamp_mt), quant_val_vt)
L_dt_lga <- data.table(grp = paste0("X", 1:Nclass),
                       grp_low = L_vt_lga[1:Nclass],
                       grp_up = L_vt_lga[2:(Nclass+1)],
                       grp_val = L_val_vt_lga)

pixel_samp_vt <- as.numeric(pixel_postsamp_mt[, 1:100])
L_vt_pixel <- quantile(pixel_samp_vt, quant_vt)
L_vt_pixel[1] <- 0
L_vt_pixel[Nclass+1] <- 1
L_val_vt_pixel <- quantile(pixel_samp_vt, quant_val_vt)
...
save(measure_state_dt, L_dt_state,
     file = paste0("cov_strat_LonoB_SPDE_OD_measure1_state_Nclass", Nclass, ".RData"))
...
save(measure_lga_dt, L_dt_lga,
     file = paste0("cov_strat_LonoB_SPDE_OD_measure1_lga_Nclass", Nclass, ".RData"))
...
save(measure_pixel_dt, L_dt_pixel,
     file = paste0("cov_strat_LonoB_SPDE_OD_measure1_pixel_Nclass", Nclass, ".RData"))
